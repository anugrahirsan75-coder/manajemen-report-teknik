/**
 * Menyusun daftar pengingat dari baris-baris projects.
 *
 * Dipisah dari store lonceng supaya bisa dipanggil DI SISI SERVER. Lonceng ada
 * di layout, jadi ia hidup di setiap halaman untuk setiap pemakai; ketika
 * penyusunan ini masih terjadi di peramban, seluruh baris mentahnya harus
 * menyeberang lebih dulu - 1 MB setiap menit per pemakai, hanya untuk
 * menghasilkan belasan baris pengingat. Disusun di server, yang menyeberang
 * tinggal hasilnya.
 */
...
/** kind yang dipakai lonceng - payload berat (SPPBJ / Non PR PO) tidak diambil */
export const KINDS = ["rr", "docking_jadwal", "kelas_bki", "servis", "kerusakan", "anggaran", "lapor_kapal"];

const isoDari = (d: Date) =>
  `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;

export function susun(rows: any[]): Pengingat[] {
  const now = new Date();
