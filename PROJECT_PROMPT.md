...
## Modul utama (sidebar)
1. **Dashboard Anggaran** (`/dashboard`) — Resume terpisah: gabung SPPBJ + Non PR PO, RKA acuan pusat, penyerapan per Mata Anggaran (Biaya/Investasi) + per kapal, Rencana & Realisasi bulanan (3 MA biaya × 13 kapal), + **Kendali Anggaran Rutin** (pagu Persetujuan Rutin per bulan per MA vs realisasi SPPBJ kategori RUTIN: pagu/terpakai/sisa/%serap/status aman-waspada-overbudget; pagu baris bebas, isi manual/salin bln lalu/tempel Excel). Realisasi = final(SPBJ) else estimasi via `realisasiRutin`/`nilaiPengadaan`. Guardrail live di form SPPBJ (banner sisa pagu + warning overbudget saat Simpan/Kirim bila kategori RUTIN). Simpan ke Supabase `projects.payload.kind='anggaran'` (rka+rr+plafon).
