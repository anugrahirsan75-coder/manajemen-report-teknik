...
## Modul utama (sidebar)
1. **Dashboard Anggaran** (`/dashboard`) — Resume terpisah: gabung SPPBJ + Non PR PO, RKA acuan pusat, penyerapan per Mata Anggaran (Biaya/Investasi) + per kapal, Rencana & Realisasi bulanan (3 MA biaya × 13 kapal), + **Kendali Anggaran Rutin** (pagu Persetujuan Rutin per bulan per MA vs realisasi SPPBJ+NonPR kategori RUTIN/tanpa-kategori: pagu/terpakai/sisa/%serap/status; proyeksi akhir bulan; klik baris → rincian) + **Kendali Anggaran Docking** (`AnggaranDocking`: pagu Persetujuan Pusat per KAPAL+tahun per MA vs realisasi SPPBJ+NonPR kategori DOCKING utk kapal itu — `realisasiDocking` item-level per kapal; simpan `payload.docking`). Realisasi = final(SPBJ) else estimasi via `realisasiRutin`/`nilaiPengadaan`. Guardrail live di form SPPBJ (banner sisa pagu + warning overbudget saat Simpan/Kirim bila kategori RUTIN). Simpan ke Supabase `projects.payload.kind='anggaran'` (rka+rr+plafon).
1c. **Profil Armada** (`/armada`, `/armada/[id]`) — VIEW-ONLY showcase kapal (terpisah dari input): grid kartu → halaman profil read-only keren (hero gradient, identity strip, highlight stats GT/LOA/B/Daya, General Data, Main Dimension + diagram SVG kapal, Machinery 3 kartu ME/AE/Gearbox, kartu file inventaris, tombol Cetak + Edit→`/kapal?open=<id>`). Data sama (`useKapalDb`). Menu Sidebar "Profil Armada".
1b. **Ship Database** (`/kapal`) — INPUT/edit data partikular 13 kapal (vessel particulars): General Data, Main Dimension, Main/Auxiliary Engine, **Gearbox**, **Daftar Inventaris (file upload)**. Grid kartu + modal editable (cincin kelengkapan %). Seed di `src/lib/kapal/types.ts` (ARIWANGAN terisi dari dokumen, lain skeleton + default ASDP). `useKapalDb()` simpan 1 row Supabase `projects.payload.kind='kapal'` (+ localStorage, merge seed). File inventaris: `src/lib/kapal/upload.ts` upload ke Storage bucket `foto` (path `inventaris/<shipId>/`), metadata di payload, klik buka tab baru, bisa update/hapus (auto-persist). Menu di Sidebar section "Resume".
